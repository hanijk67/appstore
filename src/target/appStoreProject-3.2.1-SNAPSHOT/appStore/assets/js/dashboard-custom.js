...
;(function($, window, undefined){
	...
	$(document).ready(function()
	{
		...
		// 1.4 - Fixed collapsing effect with panel tables
		...
		if($.isFunction($.fn.bootstrapWizard))
		{
			$(".form-wizard").each(function(i, el)
			{
				...
				$this.bootstrapWizard({
			  	...
			  	/*$(window).on('khan.resize', function()
			  	{
			  		$this.data('bootstrapWizard').show( _index );
			  	});*/
			});
...
function replaceCheckboxes()
{
	...
	$(".checkbox-replace:not(.khan-cb-replacement), .radio-replace:not(.khan-cb-replacement)").each(function(i, el)
	{
		var $this = $(el),
		...
		$this.addClass('khan-cb-replacement');
		...
		$input.on('change', function(ev)
		{
			if(is_radio)
			{
				//$(".khan-cb-replacement input[type=radio][name='"+name+"']").closest('.khan-cb-replacement').removeClass(checked_class);
				$(".khan-cb-replacement input[type=radio][name='"+name+"']:not(:checked)").closest('.khan-cb-replacement').removeClass(checked_class);
			}
...
function show_loading_bar(options)
{
	...
	var $ = jQuery,
		$loading_bar = $(".khan-loading-bar");

	if($loading_bar.length == 0)
	{
		$loading_bar = $('<div class="khan-loading-bar progress-is-hidden"><span data-pct="0"></span></div>');
